...
p <-   ggplot(mob_dat, aes(x=year, y=mobile_subs, col = entity, group = entity)) +
  ...
  #theme(panel.background = element_rect(fill = "#fffbf5", color = NA)) +
  theme(plot.background = element_rect(fill = '#fffbf5')) +
  theme(axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'))# +
  #geom_vline(xintercept=c(2004), linetype="longdash", color = 'red')
...
sendo <- image_read(paste0("sendo.jpg")) %>% 
  image_resize(geometry_size_percent(5)) %>% 
  image_background("#fffbf5")
image_transparent(sendo, 'white')  
